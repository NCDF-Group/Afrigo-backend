...
export function sendVerificationEmail(to: string, firstName: string, token: string) {
  ...
  const content = layout({
    ...
    body: 'Welcome to Afrigo. Confirm your email address to secure your account and unlock verified trading.',
    action: { label: 'Confirm email', url },
    footnote: 'This link expires in 24 hours. If you did not create an Afrigo account, you can ignore this email.'
  })
  return mailer.send({ to, subject: 'Confirm your Afrigo email', ...content })
}
...
export function sendPasswordResetEmail(to: string, firstName: string, token: string, staff: boolean) {
  ...
  const content = layout({
    ...
    body: 'We received a request to reset the password on your Afrigo account.',
  ...
  return mailer.send({ to, subject: 'Reset your Afrigo password', ...content })
}
...
export function sendStaffInviteEmail(to: string, firstName: string, role: string, token: string) {
  ...
  const content = layout({
    heading: `You have been invited to Afrigo Admin`,
    body: `Hello ${firstName}, you have been given ${role} access to the Afrigo operations console. Set your password to get started.`,
    action: { label: 'Set your password', url },
    footnote: 'This invitation expires in 72 hours.'
  })
  return mailer.send({ to, subject: 'Your Afrigo Admin invitation', ...content })
}

export function sendPasswordChangedEmail(to: string, firstName: string) {
  const content = layout({
    heading: 'Your password was changed',
    body: `Hello ${firstName}, the password on your Afrigo account was just changed and other devices were signed out.`,
    footnote: 'If this was not you, reset your password straight away and contact Afrigo support.'
  })
  return mailer.send({ to, subject: 'Your Afrigo password was changed', ...content })
}
